...
using crudnetcore.Repositories;
...
namespace crudnetcore
{
    public class Startup
    {
        ...
        {
            // Settings configuration for MongoDB CEH Database
            services.Configure<BooksDatabaseSettings>(
                Configuration.GetSection(nameof(BooksDatabaseSettings))
            );

            services.AddSingleton<IBooksDatabaseSettings>(
                sp => sp.GetRequiredService<IOptions<BooksDatabaseSettings>>().Value
            );
